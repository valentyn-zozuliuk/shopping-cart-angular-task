...
export class ProductsHandlerService {
    ...
    sortProductsByFilters(): Observable<Product[]> {
        ...
        if (this.priceFilterData.apply && this.typeFilterData.apply) {
            currentLink += `?type=${this.typeFilterData.currentValue}&price_gte=
            ${this.priceFilterData.currentValue[0][0]}&price_lte=${this.priceFilterData.currentValue[0][1]}`;
        } else if (this.priceFilterData.apply) {
            currentLink += `?price_gte=${this.priceFilterData.currentValue[0][0]}&price_lte=
            ${this.priceFilterData.currentValue[0][1]}`;
        ...
        if (this.priceFilterData.currentValue.length === 2 && this.priceFilterData.apply) {
            ...
            if (this.typeFilterData.apply) {
                futureLink = this.apiLink + `?type=${this.typeFilterData.currentValue}&price_gte=
                ${this.priceFilterData.currentValue[1][0]}&price_lte=${this.priceFilterData.currentValue[1][1]}`;
            } else {
                futureLink = this.apiLink + `?price_gte=${this.priceFilterData.currentValue[1][0]}&price_lte=
        ${this.priceFilterData.currentValue[1][1]}`;
    ...
    searchByQuery(query: string): Observable<Product[]> {
        ...
        let currentLink = this.apiLink + `?name_like=${query}`;
